...
// Import the virtualization plugin for editor optimization
import { createOptimizedEditor, VirtualizationPlugin } from './plugins/virtualization/index'
...
window.onload = function () {
  ...
  const instance = createOptimizedEditor(
    container,
    {
      header: [
        {
  ...
  console.log('实例: ', instance)
    // 注册虚拟化插件以优化大型文档性能
  try {
    // Register the virtualization plugin for performance optimization
    VirtualizationPlugin.register(instance)
    console.log('Virtualization plugin registered successfully')
    
    // Check if the plugin was successfully initialized by checking for the presence
    // of the debouncedRender method which should be added by DrawExtensions
    if (typeof (instance as any).debouncedRender === 'function' || 
        typeof (instance.command as any)._command?.draw?.debouncedRender === 'function') {
      console.log('Virtualization optimization is active')
    } else {
      console.warn('Virtualization plugin may not be properly initialized')
    }
  } catch (error) {
